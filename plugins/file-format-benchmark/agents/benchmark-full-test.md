...
description: Benchmarking full test executor for measuring token efficiency and answer accuracy. Reads data file (flat or nested structure), questionnaire, and answer template, answers all questions based only on data, saves results to specified output path. Tests different file formats with flat and nested data structures.
