...
class ReportGenerator {
    ...
    generateResults() {
        ...
        this.table(['Format', 'Records', 'Variant', 'Read Tokens', 'Output Tokens', 'Total', 'Tokens/Char', 'Raw Acc (%)', 'Wtd Acc (%)', 'Info/Token'], rows);
        ...
        this.table(['Format', 'Records', 'Variant', 'Read Duration (ms)', 'Read Speed (tokens/ms)'], readPerfRows);
        ...
        this.table(['Format', 'Records', 'Variant', 'Chars/Token', 'Tokens/Value', 'Tokens/Object'], structRows);
        ...
        this.table(['Format', 'Records', 'Variant', 'Total Tokens', 'Efficiency Score', 'Wtd Efficiency', 'Wasted Tokens'], effTokenRows);
        this.line();
    ...
    generateCategoryAccuracyTables() {
        ...
        recordCounts.forEach(recCount => {
            ...
            this.uniqueFormats.forEach(format => {
                const formatValidations = this.validations.filter(v => v.format === format && v.recordCount === recCount);
                ...
                categories.forEach(category => {
                    ...
                    this.line(`| ${category} | ${mandatoryAcc.toFixed(2)} | ${optionalAcc.toFixed(2)} |`);
                });
                this.line();
            });
            ...
            categories.forEach(category => {
                const deltas = this.uniqueFormats.map(fmt => {
                    const mandatory = this.validations.find(v => v.format === fmt && v.variant === 'mandatory' && v.recordCount === recCount);
                    const optional = this.validations.find(v => v.format === fmt && v.variant === 'optional' && v.recordCount === recCount);
