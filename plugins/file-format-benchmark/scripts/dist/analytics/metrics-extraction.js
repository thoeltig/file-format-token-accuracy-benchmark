...
class MetricsExtraction {
    ...
    projectsDir;
    outputFile;
    constructor(agentIdsFile, outputFile, projectsDir) {
    ...
    loadAgentIds() {
        try {
            ...
            throw new Error(`Failed to load agent IDs from ${this.agentIdsFile}:\n` +
                `Error: ${errorMsg}\n\n` +
                `Expected format:\n${JSON.stringify(exampleFormat, null, 2)}`);
        }
    }
    findTranscriptFiles(agentIds) {
        const found = new Map();
        const notFound = [];
    ...
    extractReadMetrics(transcripts, agentIdEntries) {
        const results = [];
    ...
    extractFileTokensFromTranscript(jsonlPath, agentId) {
        const results = [];
        try {
            const lines = fs.readFileSync(jsonlPath, "utf-8").split("\n");
            // Track Read tool uses with their file paths and timestamps
            const readToolUses = new Map();
    ...
    combineMetrics(readMetrics, reasoningMetricsMap) {
